...
--	=====================================================================	
--	================================================================
--	Copy any input table to any output table having the same format
--	================================================================

	drop synonym	garland.syn_input_table;
	drop synonym	garland.syn_output_table;

	create synonym	garland.syn_input_table		for jdeacutis.Common_GIS;	--	SPECIFY
	create synonym	garland.syn_output_table	for garland.Common_GIS;		--	SPECIFY

--	=============================================================

	select synonym_name, rpad(obj.owner,15) " ", rpad(object_name, 30) " ", last_ddl_time 
	from ALL_OBJECTS OBJ, ALL_SYNONYMS SYN
	where obj.object_name = syn.table_name	and obj.owner = syn.table_owner
	and obj.object_type = 'TABLE'			and syn.synonym_name = 'SYN_INPUT_TABLE';

	select synonym_name, rpad(obj.owner,15) " ", rpad(object_name, 30) " ", last_ddl_time
	from ALL_OBJECTS OBJ, ALL_SYNONYMS SYN
	where obj.object_name = syn.table_name	and obj.owner = syn.table_owner
	and obj.object_type = 'TABLE'			and syn.synonym_name = 'SYN_OUTPUT_TABLE';
