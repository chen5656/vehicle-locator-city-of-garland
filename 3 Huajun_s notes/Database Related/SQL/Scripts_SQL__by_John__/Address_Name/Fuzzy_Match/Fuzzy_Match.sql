...
--	Employ a modest amount of soundex
--	===========================================================
...
--	===========================================================
 	...
 	error_wrong				binary_integer	:= 10;	
 	error_transpose			binary_integer	:= 20;
 	error_transplace			binary_integer	:= 25;
 	error_missing			binary_integer	:= 30;
 	error_missing_misplace		binary_integer	:= 35;
 	error_extra				binary_integer	:= 40;
 	error_extra_misplace		binary_integer	:= 45;
...
--	===============================================================
	...
	conflict		binary_integer	:= 0;		
	follow1x		binary_integer;
	follow2x		binary_integer;
	follow1		char;
	follow2		char;
...
--	initialize
	...
	name1		:= translate (name1, removals, blanked);		
...
loop
	...
	if	have.here1 = have.next1						
	and	next1x in (name1_length, 01)
		have.next1	:= null;
		next1x	:= next1x + vector;
	end if;

	if	have.here2 = have.next2						
	and	next2x in (name2_length, 01)
		have.next2	:= null;
	...
	if	here2x not between front2 and back2		then		
		have.here2 := null;
		have.next2 := null;
...
--	diagnose error	
	...
	if	have.here1  = have.next2  		
	and	have.next1  = have.here2  		then
 		follow1x	:= next1x + vector;		
		follow2x	:= next2x + vector;		
	...
	elsif	have.here1  = have.next2  		then
		follow1x	:= next1x;		
		follow2x	:= next2x + vector;
	...
	elsif	have.here1	<> have.here2		then			
		follow1x	:= next1x;		
		follow2x	:= next2x;
...
--	decide whether to continue or revector 	
	...
	follow1 :=	substr(name1, follow1x, 01);
	follow2 :=	substr(name2, follow2x, 01);

	if	follow1 <> follow2				then
		conflict := conflict + 01;
	end if;

	if	test_run > 0	then
	null;
	DBMS_output.put_line( 'error:       '|| have.error   	||'    '|| follow1x ||' - '|| follow2x
							 || '     ' || series ||' / '|| conflict );
	end if;
...
--	revector scanning direction
	...
	if	revector = 0						then
		...
		if	have.error in (error_extra, error_extra_misplace)		then
			have.was_vowel1	:= have.is_vowel1;		
			
		if	have.error in (error_missing, error_missing_misplace)		then
