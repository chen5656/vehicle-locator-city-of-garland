...
--		T E M P O R A R Y 		read block side for rowid
...
while hi_node <= final_node		loop
   ...
   msg_info1 := 'b1rd	' || segm.addr_block ||' '|| segm.addr_name ||' '|| segm.addr_type 
			||' / ' || segm.esri_node  ||' / '|| lo_node ||'-'|| hi_node ||' t/'|| test_node;	
	
	open	read_hub_spoke;
	fetch	read_hub_spoke	into segm;

	if	read_hub_spoke%found 			then
		null;
	else
		goto batch_finish;
	end if;
   ...
   msg := msg || 'b1rd ';
   msg_info2 := 'b1rd	' || segm.addr_block ||' '|| segm.addr_name ||' '|| segm.addr_type 
			||' / ' || segm.esri_node  ||' / '|| lo_node ||'-'|| hi_node ||' p/'|| segm_.esri_node;	
...
begin
   ...
   msg := msg || 'mRecord ';
	...
	or	pfx1 > 2
	or	pfx2 > 2	then
		dbms_output.put_line ('WARNING = problem node ' || segm1.esri_node);
...
<<insert_record>>
	...
	if	length(segm1.addr_name) > 20							-- may 2003
	or	length(segm2.addr_name) > 20			then
		null;

	elsif	ttl_insert < do_inserts				then
	...
	if	do_corner > 0
	and	this_intersect_id like 'BB%'			then
		ttl_corner	:= ttl_corner + 01;
	...
	(
		...
		blok1.row_id,		--	block1_rowid
		blok2.row_id,		--	block2_rowid
...
--	remove street suffix
	...
	if	length(msg) > max_msg		then
		if	do_test > 0			then
			dbms_output.put_line (rpad (msg,250));
		end if;
		msg		:= null;
		msg_info1	:= null;
		msg_info2	:= null;
