...
--	===================================================================================
--	alterations
--		remove periods before looking for qualifiers

--	===================================================================================

--	create or replace	procedure	ADR_parse_address			--	C U R R E N T
...
begin
	...
	form_addr_pfx	:= upper (addr_pfx);
	form_addr_name	:= upper (addr_name);
	form_addr_type	:= upper (addr_type);
	form_addr_sfx	:= upper (addr_sfx);
	form_addr_unit	:= upper (addr_unit);
...
for	item in (select /*+ cache(adrItem) */ all * from adrItem where class = 'C')
loop
...
<<parse_prefix>>
	...
	this_name	:= translate (this_name, 'x.', 'x ');
	...
	if	space_posn - 01 <= 05							then
		...
		and	 form_addr_unit in ( 'N', 'S', 'E', 'W', 'NE', 'NW', 'SE', 'SW' )		then
			form_addr_pfx	:= form_addr_unit;
			form_addr_unit	:= null;
			parse_pfx		:= 'p';
...
<<parse_suffix>>
	...
	if	name_leng - hi_posn <= 05						then			--	NEW
--	if	name_leng - hi_posn  = 01						then
