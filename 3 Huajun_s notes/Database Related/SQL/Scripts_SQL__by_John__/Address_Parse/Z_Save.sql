...
--	choose seq 				from adrResult_Local
	do_seq	number := 4273;
...
--	addr_name	varchar2(80) :=	'w avenue g apt o'	;	
--	addr_name	varchar2(80) :=	'901 & 902  w avenue e n s'	;	
--	addr_name	varchar2(80) :=	'901 -902  w avenue e '	;	

	addr_name	varchar2(80) :=	'avenue g'	;	
	addr_num	varchar2(20) :=	''	;	
	addr_pfx	varchar2(10) :=	'w'	;		
	addr_type	varchar2(10) :=	''	;	
	addr_sfx	varchar2(10) :=	''	;	
	addr_code1	varchar2(10) :=	'apt'	;	
	addr_unit1	varchar2(20) :=	'O'	;	
...
--	===============================================	
	...
	select	addr_num, addr_pfx, addr_name, addr_type, addr_sfx,
			...
			addr_audit
	from		adr_parse_test_vw
	where		key = do_choice;

--	===============================================	

	cursor	read_adrResult_Local	is

	select	l.addr_num,	  l.addr_pfx,   l.addr_name,  l.addr_type, l.addr_sfx,
			l.addr_code1, l.addr_unit1, l.addr_code2, l.addr_unit2,
			' '	addr_audit
	from		adrResult		r,
			adrResult_Local	l
	where		r.rec_seq	= do_seq
	and		l.rowid	= r.rec_rowid;
...
--	LIMITS
	...
	code_limit		number := 03;	--	min code length
	item_limit		number := 02;	--	min item length
...
--	ITEM TABLE
	...
	type item_table	is table of	adrItem%rowtype
		index by	binary_integer;
...
--	KIND	
	...
	from_pattern	varchar2(80) :=	
			 ...
			 has_dot || has_dash || has_hash || has_slash || has_amper	||
			 has_space || chr(9)							||
			 ignore    || punctuation || reject;

	to_pattern		varchar2(80) :=	
			'AAAAAAAAAAAAAAAAAAAAAAAAAANNNNNNNNNN'				|| 
			 is_dot || is_dash  || is_hash  || is_slash  || is_amper	||
			 is_space  || is_space							||
...
--	LITERALS
	...
	duplex_code		constant	char(4) := 'DPLX';
...
--	===============================================	
	...
	item		adrItem%rowtype;
	previ		adrItem%rowtype;
...
--	===============================================	
		...
		if	prev.field = part.field
		and	prev.what  = is_unit		then
			part.what := is_unit;
			part.role := prev.role;
...
<<final>>
	...
	if	translate (form_name, 'x'||reject, 'x') <> form_name	then
	...
	if	length(form_name)  > name_limit
	...
	if	warn >= is_error		then
		addr_audit	:= '!'||warn ||' '|| addr_audit;
	end if;

	if	warning is not null	then
		warning	:= translate (warning, '/%', '\');
		addr_audit	:= addr_audit ||' '|| warning ;
	end if;
...
/*	TASKS
	...
	adrItem			punctuation / item_limit
