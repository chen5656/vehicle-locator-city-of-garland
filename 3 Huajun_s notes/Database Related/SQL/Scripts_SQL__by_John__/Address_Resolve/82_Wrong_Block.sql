...
declare
	...
	do_audit		number		:= 0;		--	update verify_code & do_qual
...
--	do_print		number		:= 01;	--	print before & after address
	...
	tolerance		number		:= 99999;	--	max gap between addr_num and lowest addr_num
									--	for which street correction is permitted

	choose_code		char			:= 'B';	--	block
	choose_verify	number		:= 33;	--	wrong block
...
cursor	read_adrResult	is
		...
		select	distinct
				...
				,rec_key, rec_seq ,rowid			--	optional

		from		adrResult

		where		rec_key in (					--	include rec_key list
...
loop
	...
	if	reject_count = 0
	and (	per_block   > 0	and accept_count = 1 )
	or  (	per_lowest  > 0	and this_block   - this_lowest <= tolerance )
	or  (	per_highest > 0	and this_highest - this_block  <= tolerance )	then
		permit_update	:= 'yes';
	else
		permit_update	:= 'no';
	end if;
