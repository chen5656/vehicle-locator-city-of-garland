...
--		explain plan	set statement_id = 'test'	for
		...
		where		this_reduced like '%' || alias_reduced || '%'	-- nov 2004
--		where		this_name 	like '%' || alias_name || '%'	
...
--	this_reduced	:= user_reduced;						-- nov 2004
...
--	if	this_mail = 'alien'		then					-- nov 2004
...
--	this_short	:= rtrim(replace (this_name||' ', 'S ', ' '));		-- nov 2004

	this_leading  := rpad (this_name, 01) || '%';

--	if	this_mail = 'alias'							-- nov 2004
--	and	this_reduced is null		then	

		this_reduced  := nvl(adr_compress_name (this_name, '1'),' ');

	if	this_mail = 'alias'		then					-- nov 2004
...
<<set_return_method>>
	...
	or	ignore_count > 0					then
		...
		if	alias.alias_name is not null		then			-- jan 2005
			edit_name	:= replace (this_name, alias.alias_name, alias.addr_name);
		else
			edit_name	:= nvl (alias.addr_name, this_name);
		end if;
