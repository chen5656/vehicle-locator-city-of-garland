...
--	========================================================================
	...
	create or replace	package body	garland.Finds_Address		As
...
--	========================================================================
	...
	that_num			binary_integer;
	that_pfx			varchar2(10);
	that_name			varchar2(40);
	that_type			varchar2(10);
...
--	dbms_output.put_line ('             /' || '' ||'/'|| '' ||'/'|| '' ||'/'|| '' ||'/'|| '' );
	dbms_output.put_line (' ');
		...
		address :=	rpad(look.addr_pfx,02)  || rpad(look.addr_name,21) || rpad(look.addr_type,05) || rpad(look.addr_sfx,02); 
...
--		line.addr_sfx		:= look.addr_sfx;	
		...
		if	text1 = 'A'					then
			address		:= rpad(look.addr_num,06) || address || rpad(look.addr_id,07) || rpad(look.parcel_id,07);
--			line.addr_num	:= look.addr_num;
--			line.location1_id	:= look.addr_id;	
--			line.location2_id	:= look.parcel_id;	

		elsif	text1 = 'B'					then
			address		:= rpad(look.addr_block,06) || address || rpad(look.block_id,07);
--			line.addr_num	:= look.addr_block;
--			line.location1_id	:= look.block_id;	
--			line.location2_id	:= null;	
		else
			address	:= rpad(' ',06) || address || rpad(look.street_id,07);
--			line.location1_id	:= look.street_id;	
--			line.location2_id	:= null;	
		end if;

		address_entry (fetch_seq) := address;

		intersect1 (fetch_seq)	:= ltrim(rtrim(look.x1_pfx ||' '|| look.x1_name ||' '|| look.x1_type 
							||' '||  look.x1_sfx ));

		intersect2 (fetch_seq)	:= ltrim(rtrim(look.x2_pfx ||' '|| look.x2_name ||' '|| look.x2_type 
							||' '||  look.x2_sfx ));
...
--		Address_Entry (fetch_seq) := Line;
	...
	dbms_output.put_line ('Line Address /' || look.addr_num  ||'/'|| look.addr_pfx ||'/'|| look.addr_name ||'/'|| look.addr_type ||'/'|| look.addr_sfx );
