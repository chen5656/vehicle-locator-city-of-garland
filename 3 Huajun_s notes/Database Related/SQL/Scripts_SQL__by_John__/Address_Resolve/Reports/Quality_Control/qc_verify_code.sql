...
(
	select 'x'		from	garland.trvehrcl		r
...
(
	select 'x'		from	garland.cdreladr		a
...
(
	select	 'x'	
	from		garland.adrBlock		s
--	from		garland.trvehrcl		s
	where		v.addr_name	= 	s.addr_name
	and		v.addr_type	in  (	s.addr_type, ' ')	
	and		v.addr_pfx	in  (	s.addr_pfx,  ' ')	
	and		v.addr_sfx	in  (	s.addr_sfx,  ' ')	
