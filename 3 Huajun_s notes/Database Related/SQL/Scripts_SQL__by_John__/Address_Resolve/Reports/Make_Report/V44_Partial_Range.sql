...
declare
		...
		spec_verify		number		:= 44;
		spec_label1		char(16)	:= 'partial range';
		spec_label2		char(16)	:= 'valid   range';
...
cursor	read_address		is
		...
		select --+ index_desc (trvehrcl) 
			distinct addr_name, addr_type, addr_pfx, addr_sfx, addr_lo, addr_hi
		from	garland.trvehrcl
		where	addr_name	= find_name
		and	addr_lo	<= find_num
		and	addr_hi	>= find_num;
...
for 	look in read_adrResult				loop
	...
	dbms_output.put_line (  spec_label1 || lpad(look.addr_num,06) ||' '||   lpad(' ',05) ||' '||
...
for	scan in read_address		loop
	...
	dbms_output.put_line (  spec_label2 || lpad(scan.addr_lo,06) ||' '|| lpad(scan.addr_hi,05) ||' '||
