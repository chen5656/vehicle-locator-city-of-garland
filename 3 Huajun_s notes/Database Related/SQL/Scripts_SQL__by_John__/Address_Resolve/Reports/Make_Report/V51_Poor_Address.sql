	set echo off;
--	===============================================================================
--	display "poor address" choices
--	===============================================================================

--	===============================================================================

	select count(*) "TOTAL"  from adrResult  where verify_code = 51;
...
declare
		...
		spec_verify		number		:= 51;
		spec_label1		char(16)	:= 'poor    address';
...
for 	look in read_adrResult				loop
	...
	dbms_output.put_line (  spec_label1 || 
					lpad(rtrim(ltrim(look.addr_num,' 0')),06) ||' '|| 
...
for	scan in read_address		loop
	...
	dbms_output.put_line (  spec_label2 || 
					lpad(scan.addr_num, 06) ||' '|| 
					rpad(scan.addr_pfx, 02) ||' '||
