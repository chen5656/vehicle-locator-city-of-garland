...
--	street qualifier correction weights
	...
	accept_weight binary_integer	:= append + remove + remove + 01;
	...
	change_unq	number	:= 1;		-- DEC 2004
		...
		adr_type		number	:= change;
		adr_pfx		number	:= change;
...
--	then "permit change" of prefix, suffix, and type according to "address" table.

--	base match		match on name and number  (exact match, range match, 100-block match) 
--	part match		match on name and number, possibly incomplete, but no conflicts
