...
declare
	...
	do_filter			number	:= 0;			-- ignore dupes
...
cursor	read_result			is
		...
		where		addr_code2 like '!%'
		order by	addr_code2, form_name, form_type, form_pfx, form_sfx, addr_num;
...
while	read.do_qual = this_do_qual			loop	
	...
	dbms_output.put_line (
							ltrim(read.addr_code2,'!') 
					||' / ' ||	rpad(read.do_qual,    20) 
					||' / ' ||	rpad(read.verify_code,03) 
