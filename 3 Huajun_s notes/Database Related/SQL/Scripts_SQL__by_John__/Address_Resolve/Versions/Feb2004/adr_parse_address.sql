...
--	===============================================	
--	oct 2003    
--		Ignore repeated street type (ex. "C Avenue Ave", "Avenue C Ave")
--		If unit info exists w/out street number, then indicate non-address (!70)
--		If highway number found, then display warn_hiwy
--		If either street number or name exists, then a number is a unit
--		Remove search adrItem restriction

--	feb 2004
--		Address item is business only if part.field < is_type and pointers = 0

--	===============================================	
--		Address Parsing Routine
--	===============================================	

--	REMOVE  do seq & do choice	<<<<<<===========
...
--	create or replace procedure	garland.ADR_parse_address
	create or replace procedure	garland.TST_parse_address
(
	...
	addr_audit		in out	varchar2,
	addr_ignore		in		varchar2	default ' '
)

as
	do_compile	number   := 0;

--	choose key to be traced
	do_choice	number   := 0;
...
(
	text			varchar2(80),
	before		char,			--  space or non-space
	kind			char,			--  number,alpha,punctuation
	field			number,		--  which input field
	what 			number,		--  role class
	role 			number,		--  number,prefix,name,type,etc
	width			number		--  text length
);
	part			part_record;	
...
--	ITEM CLASS		(on AdrItem table)

	is_compass_item	char	:= 'D';	--  direction
	is_type_item	char	:= 'T';
	is_code_item	char	:= 'C';
	is_highway_item	char	:= 'H';

	is_highway_type	varchar2(10) := 'FRWY';		-- oct 2003
...
--	===============================================	
	...
	if	item.class in ('B', 'K')
	and	part.field < is_type										-- feb 2004
	and	type_idx + suffix_idx + code1_idx + code2_idx + unit1_idx + unit2_idx = 0	-- feb 2004
	then
		warn := is_non_address;
-- msg := msg || ' v0';
		exit;
...
--			or	was_unit   > 0				
			...
			and	number_idx + name_idx > 0	)		-- oct 2003
--			and	number_idx > 0			)
...
--	901 A Avenue Ave
			if	type_idx > 0
			and	item_entry(type_idx).name <> item.name	then			-- oct 2003
				redo_role	:= is_name;			
...
--	901 Ave E
			if	previ.standard = 'AVE'
			and	prev.role = is_type		then					-- oct 2003
				part.role := is_name;
...
--	11901 STATE HIGHWAY 66								-- oct 2003
	elsif	part.kind	 = is_numeric
	and	name_count	 > 0
	and	allow_unit	 > 0	
	and	item.class		= is_highway_item
	and	previ.standard	= is_highway_type	
...
*/
	...
	if 	warning like warn_duplex
	or 	warning like warn_highway
	then
		null;
	elsif	nvl (form_num,'0') <= '0'				-- oct 2003			
