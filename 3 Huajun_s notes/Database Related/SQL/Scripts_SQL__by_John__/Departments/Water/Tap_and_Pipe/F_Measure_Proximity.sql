...
--	=========================================================
	...
	if	arm1_dx > 0
	and	abs(pipe_slope) = abs(arm1_dy / arm1_dx)		then
		goto	end_loop;

	elsif	pipe_dx = 0					then

		if	read_pipe_x1 = read_tap_x	then
			goto	end_loop;
		end if;
	...
	elsif	pipe_dy = 0					then

		if	read_pipe_y1 = read_tap_y	then
			goto	end_loop;
		end if;
...
--	does the tap lie beyond the end of the pipe
	...
	if	inner1 * inner2 > 0				then
		...
		if	arm1_length	< arm2_length		then
			drop_x	:= read_pipe_x1;
			drop_y	:= read_pipe_y1;
			trace		:= 10;
		else
...
(
	...
	if	trace = 0
	and	drop_length > 0		then
		if	(read_tap_y - drop_y)   / (read_tap_x - drop_x)		-
			(read_tap_y - extend_y) / (read_tap_x - extend_x)	between -01 and +01
		then
			null;
		else
...
<<end_loop>>
	...
	dbms_output.put_line ('Error_Count  = '	|| error_count);
