...
--	=====================================================================
	create or replace	package body  garland.SCRUB_ADDRESSES	AS
--	=====================================================================
...
--	=====================================================================
	...
	stop_seq			binary_integer		:= scrub_rule.capacity;
	stop_count			binary_integer		:= scrub_rule.capacity;
...
--	commit_size			binary_integer	:= 1000;
	...
	match_weight		binary_integer	:= scrub_rule.match;
	blank_weight		binary_integer	:= scrub_rule.blank;
	append_weight		binary_integer	:= scrub_rule.append;
	change_weight		binary_integer	:= scrub_rule.change;
	remove_weight		binary_integer	:= scrub_rule.remove;
	forbid_weight		binary_integer	:= scrub_rule.match * - 10;

	like_non_address		varchar2(3)	:= '%'|| scrub_rule.mark_non_address;
	like_non_garland		varchar2(3)	:= '%'|| scrub_rule.mark_non_garland;
	like_non_complete		varchar2(3)	:= '%'|| scrub_rule.mark_non_complete;
	like_verify_only		varchar2(3)	:= '%'|| scrub_rule.mark_verify_only;
	like_parse_error		varchar2(3) := 	   scrub_rule.mark_parse_error || '%';

	parse_default		varchar2(90) := scrub_rule.parse_default;
	signals			varchar2(20) := '&@%#\/' || chr(47);
--	chr(47) = ampersand

	process_qualifier		binary_integer	:= scrub_rule.do_street + scrub_rule.do_block	+
							   scrub_rule.do_range  + scrub_rule.do_address;
...
--	========================================
	...
	read_num			scrub_result.appl_num%type;	
	read_pfx			scrub_result.appl_pfx%type;
	read_name			scrub_result.appl_name%type;
	read_type			scrub_result.appl_type%type;
	read_sfx			scrub_result.appl_sfx%type;
	read_code1			scrub_result.appl_code1%type;
	read_unit1			scrub_result.appl_unit1%type;
	read_code2			scrub_result.appl_code2%type;
	read_unit2			scrub_result.appl_unit2%type;
	read_id			varchar2(20);
	read_mslink			binary_integer;						

	this_num			scrub_result.addr_num%type;
	this_pfx			scrub_result.addr_pfx%type;
	this_name			scrub_result.addr_name%type;
	this_type			scrub_result.addr_type%type;
	this_sfx			scrub_result.addr_sfx%type;
	this_bldg			scrub_result.addr_unit1%type;

	best_pfx			scrub_result.addr_pfx%type;
	best_name			scrub_result.addr_name%type;
	best_type			scrub_result.addr_type%type;
	best_sfx			scrub_result.addr_sfx%type;
	best_id			varchar2(20);
...
--	========================================
	...
	OUT 	garland.SCRUB_RESULT%rowtype;	
...
cursor	read_app_table		is
	...
	select *
	from	garland.SCRUB_RESULT_INPUT
...
--	end if;
	...
	if  (	out.do_name = 'A'	 and	scrub_rule.do_type_A = scrub_rule.no )	
	or  (	out.do_name = 'B'	 and	scrub_rule.do_type_B = scrub_rule.no )

	or	alias_weight < scrub_rule.do_alias_weight			then		
...
-- ****
	...
	and	alias_method > ' '	
	and	nvl(verify_id,0)	> scrub_rule.non_address		then
...
begin
	...
	if	step	= scrub_rule.do_street			then		
...
begin
	...
	if	phase = street_phase					then
		...
		elsif	alias_result	= scrub_rule.mark_verify_only		then		

			return decide_loop;

		elsif	scrub_rule.do_type_A    >= scrub_rule.yes	
		and	strt.addr_reduced  = reduced_name
		and	alias_result is null				then		
			this_weight		:= reduced_weight;
			this_name_chg	:= 'A';

		elsif	scrub_rule.do_type_B    >= scrub_rule.yes	
		and	alias_result is null				then		
...
--	and	prev_score		<= permit_change
	and	work.rule_unq_pfx <= scrub_rule.change_unq		then
		accept_prefix := 01;						
...
--	and	prev_score		<= permit_remove	
	and	work.rule_unq_pfx <= scrub_rule.remove_unq		then
		accept_prefix := 01;						
		out.addr_pfx := ' ';
...
--	and	prev_score		<= permit_append
	and	work.rule_unq_pfx <= scrub_rule.append_unq		then
		accept_prefix := 01;						
		out.addr_pfx := unique_prefix;
...
--	and	prev_score		 <= permit_change
	and	work.rule_unq_type <= scrub_rule.change_unq		then
		accept_type	  := 01;						
	...
	and	match_type 	   = 0
--	and	prev_score		 <= permit_remove
	and	work.rule_unq_type <= scrub_rule.remove_unq		then
		accept_type	  := 01;						
		out.addr_type := ' ';
	...
	and	unique_type    > ' '					
--	and	prev_score		 <= permit_append	
	and	work.rule_unq_type <= scrub_rule.append_unq		then
		accept_type	  := 01;						
		out.addr_type := unique_type;
...
--	and	prev_score		<= permit_change
	and	work.rule_unq_sfx <= scrub_rule.change_unq		then
		accept_suffix := 01;						
	...
	and	match_suffix   = 0					
--	and	prev_score		<= permit_remove
	and	work.rule_unq_sfx <= scrub_rule.remove_unq		then
		accept_suffix := 01;						
		out.addr_sfx  := ' ';
	...
	and	unique_suffix > ' '				
--	and	prev_score		<= permit_append
	and	work.rule_unq_sfx <= scrub_rule.append_unq		then
		accept_suffix := 01;						
		out.addr_sfx  := unique_suffix;
...
begin
	...
	if	out.addr_pfx   = read_pfx					then
		...
		if	work.rule_pfx <= scrub_rule.append				then
--		and	prev_score	  <= permit_append			then
	...
	and	read_pfx	  < '0'						then
		if	work.rule_pfx <= scrub_rule.remove				then
--		and	prev_score	  <= permit_remove			then
	...
	and	read_pfx	  > '0'						then
		if	work.rule_pfx <= scrub_rule.change				then
--		and	prev_score	  <= permit_change			then
	...
	if	out.addr_type   = read_type					then
		...
		if	work.rule_type <= scrub_rule.append				then
--		and	prev_score	   <= permit_append			then
	...
	and	read_type	   < '0'						then
		if	work.rule_type <= scrub_rule.remove				then
--		and	prev_score	   <= permit_remove			then		
	...
	and	read_type	   > '0'						then
		if	work.rule_type <= scrub_rule.change				then
--		and	prev_score	   <= permit_change			then
	...
	if	out.addr_sfx   = read_sfx					then
		...
		if	work.rule_sfx <= scrub_rule.append				then
--		and	prev_score    <= permit_append			then
	...
	and	read_sfx	  < '0'						then
		if	work.rule_sfx <= scrub_rule.remove				then
--		and	prev_score	  <= permit_remove			then
	...
	and	read_sfx	  > '0'						then
		if	work.rule_sfx <= scrub_rule.change				then
--		and	prev_score    <= permit_change			then
...
begin
	...
	if	retry = 0							then			
		if	length(best_name) in (2,3)			then
			out.verify_code := scrub_rule.non_address;
		end if;
	end if;

	if	out.verify_code >= scrub_rule.non_address
	or	alias_result	 = scrub_rule.mark_verify_only	then			
		retry			:= -1;
	...
	if	retry = 0
	and	scrub_rule.do_type_C >= scrub_rule.yes				then
...
--		if	count_best = 01								
		if	count_best <= scrub_rule.do_type_C						
...
--	test for trailing numbers
		...
		if	out.verify_code = scrub_rule.non_address			then
...
--	elsif	phase = street_phase						
	elsif	step  = scrub_rule.do_street
...
begin
	...
	if	out.verify_code < scrub_rule.valid_name	
	and	out.verify_code > scrub_rule.non_address		then		
		major		:= out.verify_code;

	elsif	adr.fetch_count > 0				then
		major 		:= scrub_rule.address_match;
		if	adr.match_count = 1			then
			minor	:= scrub_rule.full_match;	
		elsif	adr.match_count > 1			then
			minor	:= scrub_rule.partial_match;	
		end if;
		final_weight	:= address_weight;

	elsif	seg.fetch_count > 0				then
		major 		:= scrub_rule.segment_match;
		...
		if	nvl (out.addr_num, '0') > '0'		then
			major 	:= scrub_rule.orphan_match;
		else

			major 	:= scrub_rule.street_match;
		end if;
	...
	if	major < scrub_rule.valid_name				then
		null;

	elsif	major is null					then
		if	name_count = 01				then
			major	:= scrub_rule.valid_name;
		elsif	out.addr_num > '0'			then		
			major := scrub_rule.non_garland;
		else
			major	:= scrub_rule.non_address;				
		end if;		
...
--		best_weight := this_weight;
	...
	if	this_weight < 3				then
		minor	:= scrub_rule.poor_match;

	elsif	best_weight < 3				then
		minor	:= scrub_rule.mixed_match;

--	elsif	major < scrub_rule.block_match
--	and	match_count > 1				then
--		minor	:= scrub_rule.partial_match;

	else
		minor	:= scrub_rule.wrong_match;
	end if;
...
--	<10>		Initialize Input
	...
	if	out.verify_code < scrub_rule.valid_name			
	and	scrub_rule.do_alien  * scrub_rule.do_parse * scrub_rule.do_name = 0	then
	...
	elsif	scrub_rule.do_reset = scrub_rule.reset_to_parsed	then
		out.addr_name	:= null;
		out.do_alias	:= '';
		out.do_name		:= '';
		out.do_qual		:= '';
	...
	elsif	scrub_rule.do_name  > 0					then	
		out.do_name		:= '';
		out.do_qual		:= '';

	elsif	scrub_rule.do_street > 0				then
		out.do_qual		:= '';
	end if;

	if	out.verify_code < scrub_rule.valid_name		then				
		out.do_name			:= '';
		out.do_qual			:= '';
	end if;
...
--	<20>		Unparsed User Exit	
	...
	if	scrub_rule.do_unparsed_exit >= scrub_rule.yes	then

		garland.SCRUB_ADDRESS_UNPARSED
		(read_num,	 read_pfx,	 read_name,	 read_type,	read_sfx,
		...
		if	verify_id < scrub_rule.valid_name	then		
...
--	<30>		Match Unparsed Alias (Alien)
	...
	if	scrub_rule.do_alien >= scrub_rule.yes			then
		alias_method := 'alien';
...
--	<40>		Parse Address
	...
	if	scrub_rule.do_parse > 0			then
		...
		parse_method := scrub_rule.parse_options;					
...
--			goto	initialize_output;					
		...
		elsif	scrub_rule.do_parse = scrub_rule.safe_parse				
...
--	<50>		Parsed User Exit
	...
	if	scrub_rule.do_parsed_exit >= scrub_rule.yes	then

		garland.SCRUB_ADDRESS_PARSED
		(read_num,	read_pfx,	read_name,	read_type,	read_sfx,
		 read_code1,read_unit1, read_code2, read_unit2,
...
<<match_alias>>
	...
	if	scrub_rule.do_alias >= scrub_rule.yes	then
		alias_method := 'alias';
--	------------------------------------------------
		Match_Alias_Street;					
--	------------------------------------------------
...
--	<80>
	...
	or	scrub_rule.do_name + process_qualifier = 0	then	
		return	decide_insert;
	end if;
...
<<process_phase>>
	...
	if	alias_result like like_verify_only	then			
		...
		append_weight	:= scrub_rule.append;
		remove_weight	:= scrub_rule.remove;
		change_weight	:= scrub_rule.change;
	end if;
...
for steps in 1..5						loop
	...
	if	step in (scrub_rule.do_name, scrub_rule.do_street)	then		
		phase				:= street_phase;
		work.rule_unq_type	:= scrub_rule.str_unq_type;
		work.rule_unq_pfx		:= scrub_rule.str_unq_pfx;
		work.rule_unq_sfx		:= scrub_rule.str_unq_sfx;
		work.rule_type		:= scrub_rule.str_type;
		work.rule_pfx		:= scrub_rule.str_pfx;
		work.rule_sfx		:= scrub_rule.str_sfx;

	elsif	step = scrub_rule.do_block				then
		phase			:= block_phase;
		work.rule_type	:= scrub_rule.blk_type;
		work.rule_pfx	:= scrub_rule.blk_pfx;
		work.rule_sfx	:= scrub_rule.blk_sfx;

	elsif	step = scrub_rule.do_range				then
		phase			:= segment_phase;
		work.rule_type	:= scrub_rule.rng_type;
		work.rule_pfx	:= scrub_rule.rng_pfx;
		work.rule_sfx	:= scrub_rule.rng_sfx;

	elsif	step = scrub_rule.do_address				then
		phase			:= address_phase;
		work.rule_type	:= scrub_rule.adr_type;
		work.rule_pfx	:= scrub_rule.adr_pfx;
		work.rule_sfx	:= scrub_rule.adr_sfx;
	else	
		goto	end_phase_loop;
	end if;

	if	alias_result = scrub_rule.mark_verify_only	then		
...
/*
	if	phase = address_phase					then
		prev_score		:= scrub_rule.permit_adr_basis;
		permit_append	:= scrub_rule.permit_adr_append;
		permit_remove	:= scrub_rule.permit_adr_remove;
		permit_change	:= scrub_rule.permit_adr_change;
   	elsif	phase = segment_phase					then
		prev_score		:= scrub_rule.permit_rng_basis;
		permit_append	:= scrub_rule.permit_rng_append;
		permit_remove	:= scrub_rule.permit_rng_remove;
		permit_change	:= scrub_rule.permit_rng_change;
   	elsif	phase = block_phase					then
		prev_score		:= scrub_rule.permit_blk_basis;
		permit_append	:= scrub_rule.permit_blk_append;
		permit_remove	:= scrub_rule.permit_blk_remove;
		permit_change	:= scrub_rule.permit_blk_change;
   	elsif	phase = street_phase					then
		prev_score		:= 0;
		permit_append	:= scrub_rule.permit;	
		permit_remove	:= scrub_rule.permit;
		permit_change	:= scrub_rule.permit;
	end if;
   	if	prev_score	= scrub_rule.do_street		then
		prev_score := str.match_score;
	elsif	prev_score	= scrub_rule.do_block		then
		prev_score := blk.match_score;
	elsif	prev_score	= scrub_rule.do_range		then
		prev_score := seg.match_score;
	elsif	prev_score	= scrub_rule.do_address		then
		prev_score := adr.match_score;
	else
		prev_score	:= scrub_rule.permit;
	end if;
*/
...
--	<130>
	...
	if	step		= scrub_rule.do_street				
	and	step - 1	= scrub_rule.do_name
	and	0		< scrub_rule.do_name 		then
...
--	------------------------------------------------
	...
	if	best_score  <  scrub_rule.full_match
	and	this_type	= read_type	
	and	this_pfx	= read_pfx
	and	this_sfx	= read_sfx			then
		best_score	:= scrub_rule.full_match;

	elsif	best_score  <  scrub_rule.part_match
	and	this_type	in (' ', read_type)
	and	this_pfx	in (' ', read_pfx)
	and	this_sfx	in (' ', read_sfx)	then
		best_score	:= scrub_rule.part_match;

	elsif	best_score  <  scrub_rule.base_match	then
		best_score	:= scrub_rule.base_match;
	end if;
...
--	------------------------------------
	...
	and	scrub_rule.accept_weight < this_weight		then	
...
<<reconsider>>
	...
	elsif	step = scrub_rule.do_name			then
--	------------------------------------------------
--	<510>
...
--	<610>
	...
	if	retry  = 11						then
		retry := 12;
		if	scrub_rule.do_alias = scrub_rule.alias_fuzzy		then			
			read_num	:= out.addr_num;			
			read_pfx	:= out.addr_pfx;
			read_name	:= out.addr_name;
			read_type	:= out.addr_type;
...
<<insert_result>>
	...
	if	scrub_rule.keep_unparsed = scrub_rule.no			then
		out.appl_name	:= null;
		out.appl_type	:= null;
		out.appl_pfx	:= null;
		out.appl_sfx	:= null;
	...
	if	scrub_rule.keep_parsed = scrub_rule.no			then
		out.form_name	:= null;
		out.form_type	:= null;
		out.form_pfx	:= null;
		out.form_sfx	:= null;
	...
	if	scrub_rule.keep_unparsed = scrub_rule.yes				
	and	out.verify_code = scrub_rule.non_address		then
		out.addr_name	:= out.appl_name;
		out.addr_type	:= out.appl_type;
		out.addr_pfx	:= out.appl_pfx;
...
--		out.do_alias	:= null;	
	...
	if	do_insert > 0					then
		...
		insert into	garland.SCRUB_RESULT
		select			
...
--			row.row_id,
		...
		if	total_record  < scrub_rule.capacity		then
