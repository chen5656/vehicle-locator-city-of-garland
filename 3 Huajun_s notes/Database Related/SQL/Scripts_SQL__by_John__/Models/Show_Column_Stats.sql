...
--	<<<<<<<<<<<<  SPECIFY TABLE NAMES  <<<<<<<<<<<<<<<
	...
	or table_name like	'NSA_CODE_INSPECT'
...
--	explain plan set statement_id = 'aaa'	for
	...
	select '	truncate table   garland.gis_temp1   reuse storage;' || chr(10)	" " from dual
	union all
	...
	union all
	select '	' ||
		'insert into garland.gis_temp1  (item01, item02, item03, item04)   '	|| chr(10)		||
	...
	union all
	select '	analyze table garland.gis_temp1 compute statistics;'	" " from dual
	union all
	select '	'									" " from dual
	;
