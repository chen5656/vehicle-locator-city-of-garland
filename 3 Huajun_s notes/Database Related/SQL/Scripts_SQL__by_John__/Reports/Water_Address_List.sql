...
(
		...
		select addr_name, ' ', addr_pfx, ' ', addr_num, parcel_id, y_coord
		from	 garland.cdreladr
		where	addr_num <> 9999

		and ( addr_name, addr_num, addr_pfx )	in
(
		select addr_name, addr_num, addr_pfx
		from	 garland.cdreladr
		where	addr_pfx  > '0'
		and (	addr_type > '0'
		or	addr_sfx  > '0'	)
		group by addr_name, addr_num, addr_pfx
		having   count(*) = 1		
)

		union all

		select addr_name, ' ',' ',' ', addr_num, parcel_id, y_coord
		from	 garland.cdreladr
		where	addr_num <> 9999
